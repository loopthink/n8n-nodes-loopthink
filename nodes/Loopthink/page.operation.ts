...
/**
 * Index Page — turns a pile of rows into one page of an index.
 *
 * Deliberately does *not* duplicate what a source already does well. n8n's Data
 * Table filters dates correctly (it converts an ISO string to a Date for date
 * columns) and sorts natively, and a database does all of it better. What no
 * source here offers:
 *
 * 1. **Optional bounds.** A Data Table condition cannot be left out when a tool
 *    parameter is absent: an empty value fails with "Invalid date string ''".
 *    A bound left empty here is simply not applied, so an absent parameter needs
 *    no branching.
 * 2. **Offset paging.** The Data Table node has a limit and no offset, so a
 *    second page is not expressible with it at all.
 * 3. **The envelope.** total and hasMore, which a model needs to know whether to
 *    ask again.
 * 4. **Fewer fields.** An index should not ship whole records. Masking protects a
 *    value that travels; leaving the field out means it never does.
 *
 * Where the source can filter and sort, let it — set Sort By to the field the
 * source already ordered on, or turn on Rows Are Already Paged.
 *
 * The output is a single item, the envelope, so the branch ends in Send Result
 * with "One Object".
 */
...
export async function executePage(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
